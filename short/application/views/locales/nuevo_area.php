		...
		<section class="panel panel-primary">
			...
			<div class="panel-body">
				...
				<div class="row">
					...
					<div class="col-md-3">
						<label for="nivel">Órgano nivel</label>
					...
					<div class="col-md-3 nivel_4">
						<label for="nivel_4">Órgano nivel 4</label>
						<select name="nivel_4" id="nivel_4" autocomplete="off" class="form-control">
							<option value="">Seleccione</option>
						</select>
					</div>
				</div>
				<div class="row">
					...
					<div class="col-md-3">
						<label for="piso">Piso/Pabellon</label>
						<select name="piso" id="piso" autocomplete="off" class="form-control">
							<option value="">Seleccione</option>
							<?php
								foreach ($pisos as $piso) {
									echo "<option value='$piso->id_piso'>" . $piso->nombre . "</option>";
								} ?>
						</select>
					</div>
					<div class="col-md-3">
					...
					<div class="col-md-3">
						<label for="nombre">aforo</label>
						<input type="number" name="aforo" id="aforo" value="" max="100" min="0" autocomplete="off" maxlength="100" class="form-control" required/>
					</div>
				</div>
			</div>
			<div class="panel-footer pull-right">
				<button type="submit" id="grabarPiso" class="btn btn-success">Completar Registro</button>
			</div>
			<?php echo form_close(); ?>
		</section>
	</div>
...
<script>
	...
	$(document).ready(function () {
		...
		$('#nivel').on('change',function () {
			...
			$('#nivel_4').empty().append('<option value="">Seleccione</option>');
			if(nivel === '1') {
				$('.nivel_1').css('display','');
				$('.nivel_2').css('display','none');
				$('.nivel_3').css('display','none');
		...
		$('#nivel_3').on('change',function () {
			...
			if(nivel === '4') {
				...
				if(this.value) {
					$.ajax({
						url:         base_url + 'Organos/getNivel4/' + id,
						type:        'POST',
						cache:       false,
						contentType: 'json',
						processData: false,
						success:     function (data) {
							if(data.error === 0 || data.error === '0') {
								data['datos'].forEach(function(datos) {
									$('#nivel_4').append('<option value="'+datos.id+'">'+datos.nombre +'</option>');
								});
							} else {
								alert(data.mensaje);
							}
